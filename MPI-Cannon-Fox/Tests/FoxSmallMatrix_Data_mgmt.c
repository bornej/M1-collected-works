...
int main(int argc, char **argv){
    ...
    if (rank == 0){
        printf("---------------------------------------------------------\n");
        printf("---      Fox Small Matrix + Data Management test   ---\n");
        printf("---------------------------------------------------------\n");
    }
    ...
    snprintf(filename, 256, "%s%d%c%d", "./Results/SmallMatrixFox_DataMgmt/A_", myLineRank, '_', myColRank);
    ...
    if(rank == 0){
        snprintf(filename, 256, "%s", "./Results/SmallMatrixFox_DataMgmt/C");
        f[rank] = fopen(filename, "w");
