...
int main(int argc, char **argv){
    ...
    if (rank == 0){
        printf("---------------------------------------------------------\n");
        printf("---      Fox Large Matrix + Data Management test   ---\n");
        printf("---------------------------------------------------------\n");
    }
    ...
    snprintf(filename, 256, "%s%d%c%d", "./Results/LargeMatrixFox_DataMgmt/A_", myLineRank, '_', myColRank);
    ...
    /* Reconstitution du résultat dans Global_C à partir des blocs C */
    MPI_Gatherv(C, N*N, MPI_INT, Global_C, sendcounts, displs, myType, 0, gridComm);

    if(rank == 0){
        snprintf(filename, 256, "%s", "./Results/LargeMatrixFox_DataMgmt/C");
        f[rank] = fopen(filename, "w");
